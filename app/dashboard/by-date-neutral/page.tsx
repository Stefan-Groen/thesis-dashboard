/**
 * By Date Neutral Page
 *
 * Displays neutral articles published on a specific date
 * URL format: /dashboard/by-date-neutral?date=YYYY-MM-DD
 */
...
import { ByDateNeutralClient } from "./by-date-neutral-client"

/**
 * Fetch neutral articles for a specific date
 * MULTI-TENANT: Filters by organization_id
 */
async function getNeutralArticlesByDate(date: string): Promise<Article[]> {
  try {
    ...
    console.error('Error fetching neutral articles by date:', error)
    return []
  }
}
...
export default async function ByDateNeutralPage({ searchParams }: PageProps) {
  const params = await searchParams
  const date = params.date || new Date().toISOString().split('T')[0]
  const articles = await getNeutralArticlesByDate(date)

  return <ByDateNeutralClient key={date} articles={articles} initialDate={date} />
}
