/**
 * By Date Pending Page
 *
 * Displays pending/unclassified articles published on a specific date
 * URL format: /dashboard/by-date-pending?date=YYYY-MM-DD
 */
...
import { ByDatePendingClient } from "./by-date-pending-client"

/**
 * Fetch pending/unclassified articles for a specific date
 * MULTI-TENANT: Filters by organization_id
 */
async function getPendingArticlesByDate(date: string): Promise<Article[]> {
  try {
    ...
    const sql = `
      ...
      WHERE DATE(a.date_published) = $2
        AND (ac.status = 'PENDING' OR ac.classification IS NULL)
    ...
    console.error('Error fetching pending articles by date:', error)
    return []
  }
}
...
export default async function ByDatePendingPage({ searchParams }: PageProps) {
  const params = await searchParams
  const date = params.date || new Date().toISOString().split('T')[0]
  const articles = await getPendingArticlesByDate(date)

  return <ByDatePendingClient key={date} articles={articles} initialDate={date} />
}
