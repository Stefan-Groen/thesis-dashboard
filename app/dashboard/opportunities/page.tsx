...
/**
 * Fetch opportunity articles from our API
 */
async function getOpportunityArticles(): Promise<Article[]> {
  try {
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'

    const res = await fetch(`${baseUrl}/api/articles?classification=Opportunity&limit=1000`, {
      next: { revalidate: 30 }
    })

    if (!res.ok) {
      throw new Error('Failed to fetch opportunity articles')
    }
